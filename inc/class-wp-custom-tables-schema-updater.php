...
if ( ! class_exists( 'Wp_Custom_Tables_Schema_Updater' ) ) :

	class Wp_Custom_Tables_Schema_Updater {
			...
			public function run() {
					...
					if( $this->schema ) {
							...
							$schema_description = $this->ct_db->wpdb->get_results( "DESCRIBE {$this->ct_db->table_name}" );
							...
							foreach( $alters as $alter ) {
									...
									switch( $alter['action'] ) {
											...
											case 'ADD INDEX':
													...
													$this->ct_db->wpdb->query( "ALTER TABLE {$this->ct_db->table_name} ADD INDEX {$add_index_query}" );
													break;
											case 'MODIFY':
											...
											case 'DROP INDEX':
													// For indexes query should be executed directly
													//$this->ct_db->wpdb->query( "ALTER TABLE {$this->ct_db->table_name} DROP INDEX {$column}" );
							...
							if( ! empty( $queries ) ) {
									...
									foreach( $queries as $sql ) {
											$updated = $this->ct_db->wpdb->query( $sql );
									}
